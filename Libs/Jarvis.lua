...
ThemeManager.Themes = {
    Default = {
        ...
        AccentColor = Color3.fromRGB(255, 2, 40),
    ...
    Ocean = {
        ...
        SecondaryColor = Color3.fromRGB(3, 155, 229),
        AccentColor = Color3.fromRGB(41, 182, 246),
        BackgroundColor = Color3.fromRGB(25, 25, 25),
        ElementColor = Color3.fromRGB(33, 33, 33),
        TextColor = Color3.fromRGB(255, 255, 255),
        TextSecondaryColor = Color3.fromRGB(198, 198, 198),
    ...
    Forest = {
        ...
        SecondaryColor = Color3.fromRGB(67, 160, 71),
        AccentColor = Color3.fromRGB(102, 187, 106),
        BackgroundColor = Color3.fromRGB(25, 25, 25),
        ElementColor = Color3.fromRGB(33, 33, 33),
        TextColor = Color3.fromRGB(255, 255, 255),
        TextSecondaryColor = Color3.fromRGB(198, 198, 198),
    ...
    Purple = {
        ...
        SecondaryColor = Color3.fromRGB(142, 36, 170),
        AccentColor = Color3.fromRGB(171, 71, 188),
        BackgroundColor = Color3.fromRGB(25, 25, 25),
        ElementColor = Color3.fromRGB(33, 33, 33),
        TextColor = Color3.fromRGB(255, 255, 255),
        TextSecondaryColor = Color3.fromRGB(198, 198, 198),
...
function ThemeManager:ApplyTheme(themeName, elements)
...
-- Save Manager
local SaveManager = {}
SaveManager.Folder = "OrionConfigs"
SaveManager.ConfigFile = "config.json"
...
function SaveManager:SetConfigFile(fileName)
    self.ConfigFile = fileName
end

function SaveManager:GetConfigPath()
    return self.Folder .. "/" .. self.ConfigFile
end
...
function SaveManager:Load()
    if not isfile(self:GetConfigPath()) then
        return nil
    end
    
    local success, result = pcall(function()
        local json = readfile(self:GetConfigPath())
...
function SaveManager:Delete()
    if isfile(self:GetConfigPath()) then
        delfile(self:GetConfigPath())
        return true
    end
    return false
end
...
function SaveManager:ListConfigs()
    ...
    for _, file in pairs(listfiles(self.Folder)) do
        if file:sub(-5) == ".json" then
            table.insert(configs, file:match("([^/]+)$"))
        end
    end
    return configs
end
...
function Orion:CreateOrion(orionName)
    ...
    -- Drag functionality untuk minimize icon
    local IconDraggable = false
    local IconDragMousePosition
    local IconFramePosition
    
    minimizeIcon.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            IconDraggable = true
            IconDragMousePosition = Vector2.new(input.Position.X, input.Position.Y)
            IconFramePosition = Vector2.new(minimizeIcon.Position.X.Scale, minimizeIcon.Position.Y.Scale)
        end
    end)
    
    UserInputService.InputChanged:Connect(function(input)
        if IconDraggable == true then
            local NewPosition = IconFramePosition + ((Vector2.new(input.Position.X, input.Position.Y) - IconDragMousePosition) / Camera.ViewportSize)
            minimizeIcon.Position = UDim2.new(NewPosition.X, 0, NewPosition.Y, 0)
        end
    end)
    ...
    minimizeIcon.BackgroundColor3 = Color3.fromRGB(181, 1, 31)
    minimizeIcon.Position = UDim2.new(0.02, 0, 0.45, 0)
    ...
    minimizeIconButton.Font = Enum.Font.GothamBold
    minimizeIconButton.Text = orionName:sub(1, 1):upper()
    minimizeIconButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    minimizeIconButton.TextSize = 24
    ...
    closeLib.MouseButton1Click:Connect(function()
        ...
        if isClosed then
            ...
            MainWhiteFrame:TweenSize(UDim2.new(0, 0, 0, 0), "In", "Linear", 0.12)
            TweenService:Create(MainWhiteFrame_2, TweenInfo.new(0.10, Enum.EasingStyle.Quad, Enum.EasingDirection.In),{
    ...
    function LibraryFunctions:SetTheme(themeName)
        ThemeManager:ApplyTheme(themeName, {
            MainWhiteFrame = MainWhiteFrame,
            header = header,
    ...
    function LibraryFunctions:SaveConfig(configName)
        configName = configName or "config"
        SaveManager:SetConfigFile(configName .. ".json")
        return SaveManager:Save(configData)
    end
    
    function LibraryFunctions:LoadConfig(configName)
        configName = configName or "config"
        SaveManager:SetConfigFile(configName .. ".json")
    ...
    function LibraryFunctions:DeleteConfig(configName)
        configName = configName or "config"
        SaveManager:SetConfigFile(configName .. ".json")
        return SaveManager:Delete()
    end
    
    function LibraryFunctions:ListConfigs()
        return SaveManager:ListConfigs()
    end
    
    function LibraryFunctions:SetKeybind(key)
        UserInputService.InputBegan:Connect(function(input, gameProcessed)
            if not gameProcessed and input.KeyCode == key then
                isClosed = not isClosed
                closeLib.Image = isClosed and "rbxassetid://5165666242" or "rbxassetid://4988112250"
    ...
    function SectionHandler:CreateSection(secName)
        ...
        function ElementHandler:TextButton(buttonText, buttonInfo, callback)
            ...
            table.insert(themeElements, TextButton)
        ...
        function ElementHandler:Toggle(togInfo, default, callback)
            togInfo = togInfo or ""
            ...
            table.insert(themeElements, toggleInerFrame)
            ...
            configData[togInfo] = toggled
            
            if toggled then
                toggleBtn.BackgroundColor3 = Color3.fromRGB(181, 1, 31)
            end
        ...
        function ElementHandler:Slider(sliderin, minvalue, maxvalue, default, callback)
            ...
            sliderValue.Text = default.."/"..maxvalue
            ...
            sliderMainFrm.Size = UDim2.new(0, (default - minvalue) / (maxvalue - minvalue) * 141, 0, 10)
            
            table.insert(themeElements, sliderMainFrm)
        ...
        function ElementHandler:TextBox(textInfo, placeHolderText1, callback)
            ...
            table.insert(themeElements, texboxInner)
            ...
            TextBox.FocusLost:Connect(function(EnterPressed)
                ...
                configData[textInfo] = TextBox.Text
                TextBox.Text = ""
            end)
        end 
    ...
    LibraryFunctions.CreateSection = function(self, secName)
        return SectionHandler:CreateSection(secName)
    end
